...
@section('title', __('common.position_import_title'))

@section('breadcrumb')
    <x-breadcrumb :items="[
        ['label' => __('common.settings'), 'url' => route('settings.positions.index')],
...
@section('content')
<div>
    <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-1">{{ __('common.position_import_title') }}</h2>
    <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">{{ __('common.position_import_subtitle') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="card p-6">
            <form method="POST" action="{{ route('settings.positions.import.store') }}" enctype="multipart/form-data" class="space-y-5" novalidate>
                @csrf
                <div>
                ...
                <div class="flex flex-wrap items-center justify-end gap-3">
                    <a href="{{ route('settings.positions.import.template') }}" class="btn-secondary">
                        {{ __('common.download_template') }}
                    </a>
                    <a href="{{ route('settings.positions.index') }}" class="btn-secondary">
                        {{ __('common.cancel') }}
                    </a>
