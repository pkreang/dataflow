...
@section('title', __('common.edit_equipment_category'))

@section('breadcrumb')
    <x-breadcrumb :items="[
        ['label' => __('common.settings')],
        ['label' => __('common.equipment_categories'), 'url' => route('settings.equipment.index')],
...
<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100">{{ __('common.edit_equipment_category') }}</h2>
    ...
    <form method="POST" action="{{ route('settings.equipment.update', $equipmentCategory) }}" novalidate>
        @csrf
        @method('PUT')
        <div class="card p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                <div>
                    ...
                    <input type="text" value="{{ $equipmentCategory->auto_code }}" disabled
                ...
                <div>
                    ...
                    <input name="code" value="{{ old('code', $equipmentCategory->code) }}" required maxlength="50"
                ...
                <div>
                    ...
                    <input name="name" value="{{ old('name', $equipmentCategory->name) }}" required maxlength="255"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="2" maxlength="1000"
                              class="form-input resize-y @error('description') form-input-error @enderror">{{ old('description', $equipmentCategory->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <x-form.active-toggle name="is_active" :checked="old('is_active', $equipmentCategory->is_active)" />
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end pt-2 pb-4">
            <div class="flex items-center gap-3">
                <form method="POST" action="{{ route('settings.equipment.destroy', $equipmentCategory) }}"
