...
@section('title', __('common.edit') . ' ' . __('common.org_unit'))
...
<div>
    ...
    <form method="POST" action="{{ route('settings.org-units.update', $orgUnit) }}" novalidate>
        @csrf
        @method('PUT')
        <div class="card p-6 mb-6">
            <div class="flex items-center gap-3 mb-4">
                <h3 class="text-base font-semibold text-slate-800 dark:text-slate-200">{{ __('common.org_unit') }}</h3>
                <span class="text-xs font-mono text-slate-500 dark:text-slate-400">{{ $orgUnit->auto_code }}</span>
                @if ($membersCount > 0)
                    <span class="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
                        {{ $membersCount }} {{ __('common.members') }}
                    </span>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                <div>
                    ...
                    <input name="name" value="{{ old('name', $orgUnit->name) }}" required maxlength="255"
                ...
                <div>
                    ...
                    <select name="type" required class="form-input @error('type') form-input-error @enderror">
                        @foreach (['company','division','department','section','team'] as $t)
                            <option value="{{ $t }}" @selected(old('type', $orgUnit->type) === $t)>{{ __('common.org_type_' . $t) }}</option>
                        @endforeach
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="parent_id" class="form-input @error('parent_id') form-input-error @enderror">
                        ...
                        @foreach ($allUnits as $u)
                            <option value="{{ $u->id }}" @selected(old('parent_id', $orgUnit->parent_id) == $u->id)>
                                {{ $u->name }} ({{ $u->auto_code }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="head_user_id" class="form-input @error('head_user_id') form-input-error @enderror">
                        ...
                        @foreach ($headCandidates as $u)
                            <option value="{{ $u->id }}" @selected(old('head_user_id', $orgUnit->head_user_id) == $u->id)>
                ...
                <div>
                    ...
                    <select name="branch_id" class="form-input @error('branch_id') form-input-error @enderror">
                        ...
                        @foreach ($branches as $b)
                            <option value="{{ $b->id }}" @selected(old('branch_id', $orgUnit->branch_id) == $b->id)>{{ $b->name }}</option>
                        @endforeach
                    </select>
                    @error('branch_id')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $orgUnit->sort_order) }}" min="0"
                ...
                <div class="md:col-span-2">
                    <x-form.active-toggle name="is_active" :checked="old('is_active', $orgUnit->is_active)" />
                </div>
            </div>
        </div>
