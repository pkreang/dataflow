# RBAC Redesign — Replace Spatie with Custom R×A Model

**บทบาทไฟล์นี้:** Design spec สำหรับการรื้อ RBAC จาก Spatie Permission v7.2 มาเป็นโมเดล Resource×Action ของตัวเอง — V1 (6 สัปดาห์) + V2 (4 สัปดาห์ release ถัดไป) ออกแบบรวมในไฟล์เดียวเพื่อรับประกัน data model coherent กับฟีเจอร์ V2 ที่จะตามมา
...
**V1 Goals:**
1. **Resource × Action matrix** model — naming consistent, sparse pivot, ขยายต่อง่าย
...
| `rbac_resource_actions` | resource_id, action_id, is_default | sparse R×A pivot — กำหนดว่า action ไหน apply กับ resource ไหน | ~140 |
...
|-----|---------|----------|----------|:---:|
| read | Read | ดู | ทุก resource | ✓ |
| create | Create | สร้าง | ทุก resource | ✓ |
| update | Update | แก้ไข | ทุก resource | ✓ |
| delete | Delete | ลบ | ทุก resource | ✓ |
| manage | Manage (=CRUD) | จัดการทั้งหมด | ทุก resource (auto-derived) | ✓ |
...
Resource×Action matrix per module (collapsible), live counter, select-all per module:
...
- ✓/✗ คือ explicit grant/empty; greyed = "not applicable"
...
┌─────────────────────────────────────────────────────────────┐
│ ✓ Grant   equipment.export       Forever                      │
...
| Performance: 140 permissions × N users → check ทุก request | Eager-load effective permissions ต่อ user แล้ว cache 5 นาที; invalidate ผ่าน observer; ทำ benchmark ก่อน ship |
