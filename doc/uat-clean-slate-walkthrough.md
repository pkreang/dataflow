...
| 4 | ตั้งค่า → ผู้ใช้ | สร้าง user ≥3 ผูกแผนก/ตำแหน่ง + assign role (เช่น viewer, approver) | first_name/last_name (ไม่มี `name`), assign role ได้, ★ `auto_code` |
...
> หลังสร้าง user / role เสร็จ (ข้อ 4) แวะตรวจหน้า RBAC:
> - `ตั้งค่า → บทบาท` → ปุ่ม **"ดูภาพรวมสิทธิ์"** → ตาราง role × permission ทั้งระบบในจอเดียว ; ลองช่องค้นหา, เช็ค footnote เรื่อง `is_super_admin`
