...
7. **Shift roster รายวัน + rotation + business-day timing** (added 2026-06-12) — ระบบกะปัจจุบันคือ master + assignment ช่วงวันที่ (`user_shift_schedules`). ยังไม่มี: (ก) roster grid รายวัน/override รายวัน; (ข) การหมุนเวียนกะอัตโนมัติ (สลับสัปดาห์); (ค) ~~`work_days` ต่อ user ยังไม่ถูกใช้คำนวณวันลา~~ → **แตกเป็นข้อ 8 — requirement ยืนยันแล้ว**; (ง) escalation reminder + PM scheduling ยังนับวันปฏิทิน ไม่ข้ามวันหยุด (`SendApprovalEscalationReminders`, `PmWorkOrderGenerator`) — (ก)(ข)(ง) ทำเมื่อมี requirement attendance/โรงงานจริง

8. **`SHIFTDAYS(a, b)` — นับวันลาตามกะรายคน** (added 2026-06-12, **requirement ยืนยันแล้ว — คิวถัดไปหลัง UAT รอบปัจจุบัน**) — ฟังก์ชัน formula ตัวที่ 3 (คู่กับ DAYS/WORKDAYS, opt-in ต่อฟอร์ม ไม่เปลี่ยนฟอร์มเดิมอัตโนมัติ): จำนวนวัน inclusive หัก (1) วันหยุดบริษัท + (2) วันที่ไม่ใช่ `work_days` ของกะ **เจ้าของใบ** (submission.user_id — รองรับยื่นแทน). จุดที่ต้องแตะ: PHP/JS/Dart evaluator 3 ชุด (ต้องส่ง work_days ของเจ้าของใบไปกับหน้า create/edit + mobile schema), fallback คนไม่มีกะ = เท่า WORKDAYS, เคสเปลี่ยนกะกลางช่วง = คิดตามช่วง assignment. **คำถามที่ต้องเคลียร์กับ requirement ก่อนเขียน:** นับครึ่งวันไหม / กะข้ามคืนนับวันไหน / hint แสดง breakdown ใต้ช่องให้ผู้อนุมัติเห็นที่มาของตัวเลข
