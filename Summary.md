...
- **ชื่อผลิตภัณฑ์:** Data Flow — แพลตฟอร์ม **eForm + workflow อนุมัติ** แบบไดนามิก (แจ้งซ่อม/ลา/จัดซื้อ/เบิกค่าใช้จ่าย ฯลฯ เป็นฟอร์มในระบบ; CMMS asset/PM ถอดออกแล้ว) — ตั้งชื่อแสดงผลได้ที่ `APP_NAME`
...
- **`org_units`:** โครงสร้างองค์กรแบบ tree (node มี `type` = company/division/department/section/team, `parent_id`, `head_user_id`) — เป็นแกนเดียวขององค์กร (departments ถอดออกแล้ว ดู `doc/org-model-consolidation-spec.md`); `users.org_unit_id` FK
...
approval_workflows
  └─ approval_workflow_stages   (step_no, approver_type, approver_ref, min_approvals)
       approver_type: user / position / role / direct_manager / org_head / org_parent_head / org_n_up

org_unit_workflow_bindings      (ผูก org_unit + document_type → workflow)

document_form_workflow_policies  (ผูก form + org_unit/position → policy; priority position > org_unit > global)
  └─ document_form_workflow_ranges  (amount-based: min/max → workflow)
```

เมื่อมีการ submit เอกสาร:
1. `ApprovalFlowService::start()` หา workflow จาก **org_unit ของผู้ยื่น** (form policy → org_unit binding) หรือ amount range
2. สร้าง **`approval_instances`** record (1 ต่อ submission; `org_unit_id` snapshot)
3. สร้าง **`approval_instance_steps`** ต่อ stage (snapshot approver_type/ref; org_head/org_parent_head resolve เป็น user ตอน start)
...
- **CMMS asset/PM ถอดออกแล้ว** (equipment registry, spare parts inventory, PM plan/work order) — ผลิตภัณฑ์เป็นแพลตฟอร์ม eForm + approval ล้วน
- **ใบแจ้งซ่อม (`repair_request`)** ยังมีอยู่แต่เป็น **eForm ธรรมดา** — ยื่น+อนุมัติผ่าน `/forms` เหมือนใบลา/memo (ไม่ผูกทะเบียนอุปกรณ์)
...
- **`document_form_org_units`** — ควบคุมการมองเห็นฟอร์มตามหน่วยงาน (field-level: `document_form_fields.visible_to_org_units`)
- **`document_form_workflow_policies`** → **`document_form_workflow_ranges`** — ผูก form + org_unit/position → workflow ตาม amount range
...
- **`DataSourceRegistry`** (`app/Support/DataSourceRegistry.php`) — กำหนด data sources ที่ query ได้ (repair_requests, school_eforms, users, companies, document_form_submissions, per-form `form:{key}` ฯลฯ) พร้อม fields, aggregations, grouping, filtering, date ranges
...
- **`BranchScopingController`** + `BranchScopeService` — กรอบ isolation ข้อมูลตามสาขา (generic; module เดิม equipment/spare ถูกถอดไปกับ CMMS — ปัจจุบันยังไม่มี module ผูก)
...
- Industry templates (โรงงาน vs โรงเรียน — ทั้งคู่เป็น eForm)
...
php artisan db:seed --class=FactoryCmmsTemplateSeeder      # เทมเพลตโรงงาน eForm (แจ้งซ่อม)
php artisan db:seed --class=DevelopmentDemoSeeder          # demo users (eForm โรงเรียน)
php artisan db:seed --class=RepairApprovalDemoSeeder       # demo users (แจ้งซ่อม eForm)
...
7. ฟอร์มเอกสาร: ตรวจ field-level permissions ใน `document_form_fields` + visibility ผ่าน `document_form_org_units`
